...
for i = 1:length(Conds)
    ...
    Log1_path = fullfile(Subjfolder, sprintf('%s-Child_IC_%s.log', SubjID, Curr_cond));
    Log2_path = fullfile(Subjfolder, sprintf('%s_ChildIC_%s.log', SubjID, Curr_cond));
    ConverCsv_path = fullfile(Subjfolder, sprintf('%s_Child_IC_%s.csv', SubjID, Curr_cond));
    Output_path = fullfile(Subjfolder, sprintf('%s_ChildIC_%s_cleaned.csv', SubjID, Curr_cond));
    ...
    % --VERSION 2: Classify prediction and redundant hit after Cong--
    % [For redundant response after Cong, classify them with prediction and redundant hit + For missing InCong and missing Cong and predictition hit Cong, add a blank row]
    % 
    % For redundant response after Cong, classify them with prediction and redundant hit
    Rows2keep = false(height(Log1Table), 1); j = 1;
    while j <= height(Log1Table)
        if strcmp(Log1Table.EventType(j), 'Response')
            j = j+1; % Response of unfollowed trial
            continue;
        end 
        Rows2keep(j) = true;
        Resp_Idx = []; % Initialization to record "Response" row
        k = j+1;
        while k <= height(Log1Table)
            if strcmp(Log1Table.EventType(k), 'Response')
                Resp_Idx(end+1) = k;
                k = k+1;
            else
        ...
        Is_cong4v2 = startsWith(Log1Table.Code(j), 'Cong_');
        if Is_cong4v2
            if N_Resp >= 3 % N_response
                RT1_raw = Log1Table.Time(Resp_Idx(1)) - Log1Table.Time(j); 
                if RT1_raw <= 3500
                    Log1Table.StimType(j) = "pred_hit"; % Prediction act, RT is not counted
                else
                    Log1Table.StimType(j) = "red_hit";
                    % Rows2keep(Resp_Idx(1)) = true; % Redundant act, RT keep the first one
                end
            elseif N_Resp == 2
                Rows2keep(Resp_Idx(1)) = true; % RT keep the first one
            elseif N_Resp == 1
                Rows2keep(Resp_Idx(1)) = true;
            end
        else
            if N_Resp > 0 % For InCong
                Rows2keep(Resp_Idx(1)) = true;
            end
        end
        j =k;
    ...
    % For missing InCong and missing Cong and predictition hit Cong, add a blank row
    ...
    for j = height(Log1Table):-1:1 
        Is_incong = startsWith(Log1Table.Code(j), 'InCong_');
        Is_cong = startsWith(Log1Table.Code(j), 'Cong_');
        Is_miss = strcmp(Log1Table.StimType(j), 'miss');
        Is_red_hit = strcmp(Log1Table.StimType(j), 'red_hit'); 
        Is_pred_hit = strcmp(Log1Table.StimType(j), 'pred_hit'); 

        if Is_incong && Is_miss % For missing InCong
            Log1Table = [Log1Table(1:j, :); BlankRow; Log1Table(j+1:end, :)];
        end

        if (Is_cong && Is_miss) || (Is_cong && Is_pred_hit) || (Is_cong && Is_red_hit) % For missing Cong and prediction hit Cong
           Log1Table = [Log1Table(1:j, :); BlankRow; Log1Table(j+1:end, :)];
           % Replace back to "miss"
           % if Is_cong && Is_multi_resp_miss
           %     Log1Table.StimType(j) = "miss";
           % end
        end
   end

    % --VERSION 1: Delete redundant response but keep the first one (original)--
    % [For missing InCong and missing Cong, add a blank row + For redundant response after Cong, delete them and keep the first one]
    %
